...
class ResponseBuilder {
  ...
  constructor({ bucket, s3client, threshold, urlTTL } = {}) {
    ...
    this.bucket = bucket;
    this.s3client = s3client || new AWS.S3({ apiVersion: '2006-03-01' });
  ...
  static async _parseCompressedResponse(response) {
    try {
      ...
      const decompressed = await BPromise.fromCallback(cb => zlib.gunzip(buf, cb));
  ...
  static async _parseS3Response(response) {
    try {
      const responseData = await request(response.body);
      return new Response(JSON.parse(responseData));
    } catch (e) {
      throw new WError({
        cause: e,
        info: response.body
  ...
  async _buildCompressedResponse(response) {
    try {
      ...
      const compressedBody = await BPromise.fromCallback(cb => zlib.gzip(buf, cb));
      return new Response({
  ...
  async _buildS3Response(response) {
    ...
    try {
      const url = await BPromise
        .fromCallback(cb => this.s3client.getSignedUrl('getObject', getParams, cb));
      return new Response({
        statusCode: response.statusCode,
